...
car                         = CarClass(road, "right");
car                         = car.getVertexes();

car2                        = CarClass(road, "left");
car2                        = car2.getVertexes();
car2                        = car2.setSpeed(35/3.6);

camera                      = CameraClass(200/3.6, road.laneWidth);
camera                      = camera.getDriverPosition(car, road); % gets the camera position by estimating a car's place
keys                        = KeysClass();
...
while trial.currentBlock <= trial.nBlocks
    ...
    while trial.currentTrial <= trial.nTrials


        trial.printTrial(scrn, 3);
